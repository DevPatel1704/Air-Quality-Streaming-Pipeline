...
runs the pre-trained Random Forest Regressor to predict CO concentration,
...
async def process_air_quality(stream):
    ...
    async for event in stream:
        ...
        co_predicted = float(round(MODEL.predict(X)[0], 3))

        # Compute absolute error if actual value is available
        error = round(abs(float(co_actual) - co_predicted), 3) if co_actual is not None else None
